...
- ✨ My Lists: "Recently watched" sort order driven by what was actually played through Seanime
  - The library page's "Most recent watch" sort relies on the Continuity watch history, which is gated behind the watch-continuity setting, capped at 100 items, silently dropped once an episode passes 90 % completion, and never written at all for manual-tracking playback (Denshi's client-side mpv). None of that makes a reliable "what did I watch recently" signal, and the My Lists page had no recency sort of its own.
  - **Server:** new `media_watch_activities` SQLite table (`models.MediaWatchActivity`, one row per AniList media ID with the last episode/chapter number and a `lastWatchedAt` timestamp). It is upserted from every path that means "Seanime played or synced this": the playback manager on local-file / stream tracking start, on manual-tracking start, and after a successful AniList progress sync (covers server-side mpv/VLC/etc. and client-mpv); VideoCore on `loadedmetadata` and on episode completion (built-in player, mediastream, online/torrent streams); and the `library/anime-entry/update-progress` + `manga/update-progress` endpoints. Exposed through `GET /api/v1/library/watch-activity`. Recording is best-effort — a failed write logs a warning and never interrupts playback.
  - **AniList fallback:** the `AnimeCollection` / `MangaCollection` GraphQL queries now also fetch each list entry's `updatedAt`, regenerated with gqlgenc (`client_gen.go` only gained the `UpdatedAt` field + getter; `models_gen.go` picked up benign upstream schema comment/enum drift such as `MediaRelation.SAME_UNIVERSE`). Entries with no Seanime activity are ordered by that timestamp; entries with neither always sink to the bottom.
  - **Web:** new `RECENTLY_WATCHED(_DESC)` sort values in `lib/helpers/filtering.ts` with `MY_LISTS_ANIME_SORTING_OPTIONS` ("Recently watched" / "Least recently watched") and `MY_LISTS_MANGA_SORTING_OPTIONS` ("Recently read" / "Least recently read"). The pure helpers live in `lib/helpers/my-lists-filtering.ts` (`getEntryLastActivityTime`, `sortEntriesByLastActivity`, `buildWatchActivityMap`) and are unit-tested; `filterMyListsEntries` composes them on top of the existing `filterListEntries`. The watch-activity query is invalidated together with the collection queries when the server broadcasts a collection refresh, so the order updates right after a progress sync.
- ✨ My Lists: "Local only" filter
  - New switch next to the "Adult" toggle (both now sit in one row) that keeps only entries with local content. Anime uses the library collection's `libraryData` — the same source as the library badge on the cards — so the filter matches exactly the entries that show the badge; manga uses the downloaded-chapters list (`manga/downloads`), which is only queried while the Manga tab is active. While the local data is still loading the filter shows nothing rather than briefly flashing every entry.
  - `CollectionParams` gained a required `localOnly: boolean` (all default param objects set it to `false`); the My Lists trash button resets it and highlights when it is on. `useGetMangaDownloadsList` accepts an optional `{ enabled }` like `useGetLibraryCollection` so the download-folder scan is skipped when it isn't needed.
- Tests: `internal/database/db/media_watch_activity_test.go` (empty result, insert, upsert-bumps-timestamp, invalid ID) and `seanime-web/src/lib/helpers/my-lists-filtering.test.ts` (15 cases). Note that `seanime-web` has no test runner in `package.json`; the suite was verified with `npx vitest@3 run`.
